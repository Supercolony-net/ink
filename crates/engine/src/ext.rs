...
impl Engine {
    ...
    pub fn new() -> Self {
        Self {
            ...
            contracts: Rc::new(RefCell::new(Default::default())),
        }
    }
}
...
impl Engine {
    ...
    pub fn set_storage(&mut self, key: &[u8; 32], encoded_value: &[u8]) {
        ...
        let _ = self.database.borrow_mut().insert_into_contract_storage(
            &callee,
            key,
    ...
    pub fn random(&self, subject: &[u8], output: &mut &mut [u8]) {
        let seed = (self.exec_context.borrow().entropy, subject).encode();
